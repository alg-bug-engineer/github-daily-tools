...
STATE_FILE = SCRIPT_DIR / "storage.json"
...
def move_post_to_history(md_path: Path) -> None:
    """将已发布的 Markdown 及相关 HTML 副本移动到 history 目录。"""
    def _move_if_exists(source: Path) -> None:
    ...
    _move_if_exists(md_path)
    html_candidates = [
        md_path.with_suffix(".html"),
        md_path.with_name(f"{md_path.stem}_with_footer.html"),
    ]
    for candidate in html_candidates:
        _move_if_exists(candidate)
...
def main():
    ...
    if not POSTS_DIR.exists() or not POSTS_DIR.is_dir():
        print(f"未找到 posts 目录，请在 {POSTS_DIR} 创建文件夹并放入 .md 文件")
        sys.exit(2)
    ...
    with sync_playwright() as p:
        ...
        for idx, fp in enumerate(files_to_process, start=1):
            if publish_log.get(fp.name, {}).get("published"):
                print(f"跳过已发布: {fp.name}")
                continue
