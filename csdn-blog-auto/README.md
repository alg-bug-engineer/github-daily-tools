...
- **🧾 发布日志**：`logs/publish_log.json` 记录每篇文章的发布结果（含时间戳），自动跳过已发布文件
- **📁 历史归档**：成功发布后自动将 Markdown 及关联 HTML 文件移动到 `history/`，保持 `posts/` 仅存未发布内容
...
│   ├── history/                    # 已发布文章与生成的 HTML 归档
│   ├── logs/                       # 发布日志（publish_log.json 自动维护）
...
- `logs/publish_log.json`：记录每篇文章的发布结果、时间戳与标签信息，可用于跳过已发布内容。
- `history/`：成功发布后会自动将 Markdown 及同名 HTML 归档到此目录，保持 `posts/` 仅存待发布文章。
- 以上目录与 `storage.json` 均已在 `.gitignore` 中默认忽略，确保敏感信息不会被提交。
