...
from logging_utils import capture_run_logs
...
def _log_prefix(args) -> str:
    if args.full:
        return "pipeline_full"
    if args.explore:
        return "pipeline_explore"
    if args.curriculum:
        return "pipeline_curriculum"
    if args.article:
...
使用示例：
  ...
  python main.py --show "Transformer架构"
    ...
    log_prefix = _log_prefix(args)
    
    with capture_run_logs(log_prefix):
        try:
