...
✅ **发布记录** - 自动记录已发布文章,避免重复发布,日志保存在 `logs/publish_log.json`  
✅ **历史归档** - 发布成功后自动将 Markdown 与生成的 HTML 移动到 `history/` 目录  
✅ **智能间隔** - 默认每篇文章间隔 20 秒 + 随机延迟(可通过参数自定义)  
...
3. 逐个发布文章,每篇间隔 `--cooldown` 秒 + 随机 0-10 秒
4. 记录发布结果到 `logs/publish_log.json`,并将已发布文章移动到 `history/`
...
| 参数 | 默认值 | 说明 |
| ---- | ------ | ---- |
| `--headless` | `false` | 是否使用无头模式(`true/false`) |
| `--login-timeout` | `180` | 首次登录等待时间(秒) |
| `--skip-publish` | `False` | 只填充内容,不点击发布 |
| `--cooldown` | `20` | 每篇文章之间的基础等待秒数,实际等待 = cooldown + 随机 0-10 秒 |

## 文件说明

- `zhihu_publish.py` - 主程序脚本
- `posts/` - 存放待发布的 Markdown 文件
- `history/` - 自动归档已成功发布的 Markdown 与对应 HTML
- `logs/publish_log.json` - 发布记录日志 (自动生成,含更新时间元数据)
